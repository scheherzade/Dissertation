...
Looking at $t\_{overhead}$, with $num\_{tasks}$ tasks created, $\left\lceil{\frac{num\_{tasks}}{N}}\right \rceil$ tasks would be created on $core\_0$. If we represent the overhead of creating a task on one core with $\alpha$, $t\_{overhead}$ could be stated as $\alpha\times{\left\lceil{\frac{num\_{tasks}}{N}}\right \rceil}$. Formula~\ref{formula31} then becomes:

\begin{equation}\label{formula32}
\begin{aligned}
&execution\_time = 
\alpha\times{\left\lceil{\frac{num\_{tasks}}{N}}\right\rceil\:\:+\:\:w\_c\:\:+\:\:q}
\end{aligned}
\end{equation}
...
Meanwhile, the overhead of managing a task on a machine is shown with $\delta\times{num\_{tasks}}$. 
...
\begin{aligned}
&
\alpha\left\lceil{\frac{num\_{tasks}}{N}}\right \rceil +\delta\times{num\_{tasks}}+{w\_c+\gamma\times{w\_c}\times{(M-1)}} \:\: \text{if\:\:}num\_{tasks}>N\\
&\alpha\left \lceil{\frac{num\_{tasks}}{N}}\right \rceil +\delta\times{(N-num\_{tasks})}\times{num\_{tasks}}+{w\_c+\gamma\times{w\_c}\times{(M-1)}} \:\: \text{otherwise}
\end{aligned}
\right.
\end{aligned}
\end{equation}
