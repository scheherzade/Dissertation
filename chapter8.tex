...
While the overhead of creating a task should be constant on a specific machine architecture, we are suggesting here that the overall overhead of managing $num\_{tasks}$ tasks, is a factor($\delta$) of $N\times{N-1}$, due to the effect of work stealing. We observe this factor when the number of tasks created is greater than the number of cores, affecting the execution time as $\delta\times{N\times{N-1}}\times{num\_{tasks}}\times{\frac{1}{N}}$. 
...
We represent the overhead of creating a task on a machine with $\alpha$, and the overhead of managing a task on a machine with $\delta\times{(N-1)}\times{num\_{tasks}}$, when $num\_{tasks}>N$, and $\delta\times{(N-num\_{tasks})}\times{num\_{tasks}}$ otherwise. 
...
\begin{aligned}
&
\alpha\left\lceil{\frac{num\_{tasks}}{M}}\right \rceil +\delta\times{(N-1)}\times{num\_{tasks}}+{w\_c+\gamma\times{w\_c}\times{(M-1)}} \:\: \text{if\:\:}num\_{tasks}>N\\
