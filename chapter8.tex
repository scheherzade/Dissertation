...
While the overhead of creating a task should be constant on a specific machine architecture, we are suggesting here that the overall overhead of managing $num\_{tasks}$ tasks, is a factor of $N\times{N-1}$, due to the effect of load balancing resulted from work stealing. We observe this effect when the number of tasks created is greater than the number of cores. 

On the other hand, when number of tasks created is smaller than the number of cores, the positive load balancing effect we observed earlier with more number of tasks than cores, now becomes a negative effect due to continuous efforts of the idle cores to steal work from cores that are actually doing the work, interrupting the cache coherency constantly. We represent this effect with $\delta\times{(num\_{tasks}-1)}$.


We represent the overhead of creating a task on a machine with $\alpha$, and the overhead of managing a task on a machine with $\beta\times{N}\times{N-1}$, when $num\_{tasks}>N$, and $num\_{tasks}-1$ otherwise. 
